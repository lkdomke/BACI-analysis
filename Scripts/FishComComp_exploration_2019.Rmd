...
seine19 <- read.csv("../APECS Master repository/APECS Master repo/ALL_DATA/Lia_fish/FishSeine_2019_RAW_10-4-19.csv",
                    stringsAsFactors = FALSE, header = TRUE)
seine17 <- read.csv(url("https://knb.ecoinformatics.org/knb/d1/mn/v2/object/urn%3Auuid%3A8dba01ba-ebec-4b45-afe5-391947abcc6e"), stringsAsFactors = FALSE, header = TRUE)
...
site_names <- read.csv(url("https://knb.ecoinformatics.org/knb/d1/mn/v2/object/urn%3Auuid%3A87744ced-530b-4bae-809e-ff1012c7ae72"), stringsAsFactors = FALSE, header = TRUE)
```
...
# write out csv for noaa data
write.csv(noaa_fish, "../Data/ba_noaa_combined.csv")
```
...
# write out combined 17 and 19 dataset
write.csv(fish_contemp, "../Data/Contemp_combined_seines17-19.csv")
```
...
## 5.01 sp comp btween habs ggplot
```{r}
scrs <- scores(sp.nmds, display = 'sites')
scrs <- cbind(as.data.frame(scrs), Habitat = site_info$habitat)
...
vec.sp <- envfit(sp.nmds$points, fish.scales3[,1:69], perm = 99)
...
# with ellipses
hab <- ggplot(scrs, aes(x=NMDS1, y= NMDS2, colour = Habitat)) +
  geom_point(size = 3) +
  ...
  geom_label(data = vec.sp.dat.sig, aes(x = MDS1_sc, y = MDS2_sc, label = common), size = 8,
            inherit.aes = FALSE, angle = arrow_angle, hjust = arrow_hjust)
hab
#ggsave("hab_baci_comp.png", width = 20, height = 14, units = "in")
```
...
ggplot(scrs, aes(x=NMDS1, y=NMDS2, colour = BA)) +
  ...
  geom_text_repel(data = vec.sp.dat.sig, aes(x = MDS1_sc, y = MDS2_sc, label = common), size = 8, inherit.aes = FALSE,
...
# with ellipses
eel <- ggplot(scrs, aes(x=NMDS1, y= NMDS2, colour = Before_After)) +
  geom_point(size = 3) +
  stat_ellipse(size = 1.5) +
  xlab(label = "nMDS 1") + ylab(label = "nMDS 2") + 
  ...
  geom_segment(data = vec.sp.dat.sig, aes(x = 0, xend = MDS1_sc, y = 0, yend = MDS2_sc),
               inherit.aes=FALSE) +  # add arrows
  geom_label_repel(data = vec.sp.dat.sig, aes(x = MDS1_sc, y = MDS2_sc, label = common), 
                   size = 8,
            inherit.aes = FALSE, angle = arrow_angle, hjust = arrow_hjust)
eel
ggsave("baci_eel_comp.png", width = 20, height = 14, units = "in")
```
...
# with ellipses
kelp <- ggplot(scrs, aes(x=NMDS1, y= NMDS2, colour = Before_After)) +
  geom_point(size = 3) +
  stat_ellipse(size = 1.5) +
  xlab(label = "nMDS 1") + ylab(label = "nMDS 2") + 
  ...
  geom_segment(data = vec.sp.dat.sig, aes(x = 0, xend = MDS1_sc, y = 0, yend = MDS2_sc),
               inherit.aes=FALSE) +  # add arrows
  geom_label_repel(data = vec.sp.dat.sig, aes(x = MDS1_sc, y = MDS2_sc, label = common), size = 8,
            inherit.aes = FALSE, angle = arrow_angle, hjust = arrow_hjust)

ggsave("baci_kelp_comp.png", width = 20, height = 14, units = "in")
```
...
inset_map <- ggplot() +
  ...
  xlab("") +
  ylab("") +
  annotate(geom = "rect", ymax = 59, ymin = 54, xmax = -137, xmin = -130, fill = NA, colour = "red") +
  theme_bw()
inset_map
...
ggdraw(base_map) + 
  draw_plot(inset_map, width = 8, heigh = 8, x = -132.5, y = 57)
```
